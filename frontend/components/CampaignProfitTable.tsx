    ...
    >
      ...
      <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 2 }}>
            {data?.configured
              ? `Lucro estimado usando custos configurados (taxa não-FB: ${data.nonFbCostRate}% da receita)`
              : 'Configure seus custos em Lucro para ver o lucro estimado'}
          </p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              {['Campanha', 'Gasto FB', 'Receita FB', 'Pedidos', 'ROAS', 'Lucro Est.', 'Margem'].map(h => (
                <th key={h} style={{ padding: '8px 16px', textAlign: h === 'Campanha' ? 'left' : 'right', color: 'var(--text-faint)', fontWeight: 600, fontSize: 10, letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {loading ? (
                ...
                <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                  {Array(7).fill(0).map((_, j) => (
                    <td key={j} style={{ padding: '10px 16px' }}>
                      <div style={{ height: 12, background: 'var(--bg-hover)', borderRadius: 4, width: j === 0 ? '60%' : '40%' }} />
                    </td>
                  ))}
                </tr>
              ))
              ...
              <tr>
                <td colSpan={7} style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--text-faint)', fontSize: 13 }}>
              ...
              return (
                <tr key={row.campaign_id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                  <td style={{ padding: '10px 16px', color: 'var(--text-primary)', fontWeight: 500, maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    ...
                    {row.objective && <span style={{ marginLeft: 8, fontSize: 10, color: 'var(--text-ghost)', background: 'var(--bg-hover)', borderRadius: 4, padding: '1px 5px' }}>{row.objective}</span>}
                  </td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', color: 'var(--text-secondary)' }}>{fmt(row.spend)}</td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', color: 'var(--text-secondary)' }}>{fmt(row.fb_revenue)}</td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', color: 'var(--text-muted)' }}>{row.purchases}</td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', fontWeight: 600, color: roasColor }}>{row.roas.toFixed(2)}x</td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', fontWeight: 700, color: profitColor }}>
                    {row.configured ? fmt(row.estimated_profit) : '—'}
                  </td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', color: profitColor }}>
                    {row.configured ? `${row.margin.toFixed(1)}%` : '—'}
                  </td>
                </tr>
              )
            })}
      ...
      {!data?.configured && rows.length > 0 && (
        <div style={{ padding: '10px 20px', borderTop: '1px solid var(--border)', background: 'rgba(245,158,11,0.05)', fontSize: 11, color: '#F59E0B' }}>
          ⚠ Configure seus custos reais (COGS, frete, taxas) na aba Lucro para ver o lucro estimado por campanha.
        </div>
      )}
    </motion.div>
  )
